...
CREATE TYPE USER_ROLE AS ENUM ('STUDENT', 'INSTRUCTOR', 'STAFF', 'ADVISOR');  -- noqa
...
CREATE TABLE public.users (
    ...
    major_id INT REFERENCES public.majors (id) ON DELETE RESTRICT,
    ...
    ) DEFERRABLE INITIALLY DEFERRED -- Defer the check to the end of a user insert, where role-related columns are filled in later
);
...
-- Advisors can work for more than one department (from Constraint 6: "advisors can
...
CREATE TABLE public.courses (
    ...
    department_id NOT NULL INT REFERENCES public.departments (id) ON DELETE RESTRICT,
...
-- Enforce that an instructor cannot teach the same course twice in the same semester
-- (Constraint B.2)
...
DECLARE
    user_role USER_ROLE;
BEGIN
    -- Retrieve the role of the user with id = NEW.instructor_id
    SELECT role INTO user_role FROM public.users WHERE id = NEW.instructor_id;
    
    -- Check if the role is 'INSTRUCTOR'
    IF user_role <> 'INSTRUCTOR' THEN
        RAISE EXCEPTION 'User % is not an instructor', NEW.instructor_id;
    END IF;
...
language plpgsql
;
CREATE TRIGGER trigger_check_instructor_role
...
DECLARE
    user_role USER_ROLE;
BEGIN
    -- Retrieve the role of the user with id = NEW.student_id
...
language plpgsql
;
CREATE TRIGGER trigger_check_student_role
BEFORE INSERT OR UPDATE ON public.course_enrollments
FOR EACH ROW EXECUTE FUNCTION check_student_role();

-- Trigger function to prevent students from enrolling in the same course twice in the
-- same semester (Constraint 2.2: "Cannot take the same course twice in the same
-- semester.")
