...
export const getAllSemesters = async () => {
  ...
  export const getGPAByMajor = async (majorId: string | null) => {
    try {
      ...
      const averageGPA = parseFloat((validGpas.reduce((acc, gpa) => acc + gpa, 0) / validGpas.length).toFixed(2));

      console.log(`Major ID: ${majorId}, Highest GPA: ${highestGPA}, Lowest GPA: ${lowestGPA}, Average GPA: ${averageGPA}`);
